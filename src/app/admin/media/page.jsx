    ...
    return (
        <div className="space-y-6 relative">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-xl sm:text-2xl font-bold text-navy">Media Library</h2>
                    <p className="text-brand-muted text-xs sm:text-sm">Upload and manage all website assets.</p>
                </div>
                <button className="btn-primary w-full sm:w-auto" onClick={() => setIsUploadModalOpen(true)}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <div className="lg:col-span-1 space-y-4">
                    <div className="admin-card p-4">
                        <h3 className="text-[10px] sm:text-xs font-black text-navy uppercase tracking-widest mb-4">Folders</h3>
                        <div className="flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 scrollbar-hide">
                            {folders.map(folder => (
                                <button
                                    ...
                                    className={cn(
                                        "whitespace-nowrap flex items-center justify-between px-3 py-2 rounded-lg text-xs sm:text-sm transition-all grow lg:grow-0",
                                ...
                                >
                                    ...
                                    <span className="text-[10px] opacity-60 font-black ml-2">{folder.count}</span>
                                </button>
                            ))}
                        </div>
                        <button className="hidden lg:flex w-full mt-4 items-center justify-center gap-2 p-2 border border-dashed border-brand-border rounded-lg text-xs font-bold text-brand-muted hover:text-primary hover:border-primary/50 transition-all">
                    ...
                    <div className="admin-card p-4">
                        <h3 className="text-[10px] sm:text-xs font-black text-navy uppercase tracking-widest mb-3">Storage</h3>
                        <div className="w-full bg-brand-bg h-2 rounded-full overflow-hidden mb-2">
                            <div className="bg-primary h-full w-[65%] shadow-[0_0_8px_rgba(59,130,246,0.5)]"></div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] font-bold">
                            <span className="text-brand-muted">6.5 GB of 10 GB</span>
                            <span className="text-primary hover:underline cursor-pointer uppercase tracking-tighter">Upgrade</span>
                        </div>
                    </div>
                </div>

                <div className="lg:col-span-3 space-y-4 sm:space-y-6">
                    <div className="admin-card p-3 sm:p-4 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            <div className="flex items-center bg-brand-bg p-1 rounded-lg flex-1 sm:flex-none">
                                <button
                                    className={cn("flex-1 sm:flex-none p-1.5 rounded-md transition-colors", viewMode === "grid" ? "bg-white shadow-sm text-primary" : "text-brand-muted hover:text-navy")}
                                    onClick={() => setViewMode("grid")}
                                >
                                    <Grid className="w-4 h-4 mx-auto" />
                                </button>
                                <button
                                    className={cn("flex-1 sm:flex-none p-1.5 rounded-md transition-colors", viewMode === "list" ? "bg-white shadow-sm text-primary" : "text-brand-muted hover:text-navy")}
                                    onClick={() => setViewMode("list")}
                                >
                                    <List className="w-4 h-4 mx-auto" />
                                </button>
                            </div>
                            <button className="btn-secondary h-10 px-3 flex-1 sm:flex-none">
                                    ...
                                    <div className="aspect-square bg-brand-bg flex items-center justify-center relative border-b border-brand-border">
                                        {asset.type === "image" ? (
                                            <div className="w-full h-full bg-blue-50/50 flex items-center justify-center">
                                                <ImageIcon className="w-8 sm:w-10 h-8 sm:h-10 text-primary opacity-20" />
                                            </div>
                                        ) : asset.type === "video" ? (
                                            <div className="w-full h-full bg-purple-50 flex items-center justify-center">
                                                <Video className="w-8 sm:w-10 h-8 sm:h-10 text-purple-400 opacity-40" />
                                            </div>
                                        ) : (
                                            <div className="w-full h-full bg-emerald-50 flex items-center justify-center">
                                                <FileIcon className="w-8 sm:w-10 h-8 sm:h-10 text-emerald-400 opacity-40" />
                                            </div>
                                        )}
                                        <div className="absolute inset-0 bg-navy/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                            {asset.type === "image" && (
                                                <button className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white text-navy flex items-center justify-center hover:bg-primary hover:text-white transition-colors">
                                                    <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                                </button>
                                            )}
                                            <button className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors" onClick={() => handleDelete(asset.id)}>
                                                <Trash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                            </button>
                                        </div>
                                        <div className="absolute bottom-2 left-2 px-1 py-0.5 bg-white/90 backdrop-blur rounded text-[7px] sm:text-[8px] font-black uppercase tracking-tighter shadow-sm">
                                            {asset.type}
                                        </div>
                                    </div>
                                    <div className="p-2 sm:p-3">
                                        <p className="text-[11px] sm:text-xs font-bold text-navy truncate" title={asset.name}>{asset.name}</p>
                                        <div className="flex items-center justify-between mt-1 text-[8px] sm:text-[9px] text-brand-muted font-bold tracking-widest uppercase">
                                            <span>{asset.size}</span>
                                            <span className="hidden sm:inline">{asset.dimension}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                                        ...
                                        <tr key={asset.id} className="hover:bg-brand-bg/30 transition-colors group">
                                            <td>
                                                <div className="flex items-center gap-2 sm:gap-3 min-w-[120px]">
                                                    <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-brand-bg flex items-center justify-center shrink-0">
                                                        {asset.type === "image" ? <ImageIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" /> : asset.type === "video" ? <Video className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500" /> : <FileText className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-500" />}
                                                    </div>
                                                    <span className="text-xs sm:text-sm font-bold text-navy truncate max-w-[120px] sm:max-w-[200px]">{asset.name}</span>
                                                </div>
                                            </td>
                                            <td className="hidden sm:table-cell">
                                                <span className="text-[9px] font-black uppercase tracking-widest text-brand-muted bg-brand-bg px-1.5 py-0.5 rounded">{asset.type}</span>
                                            </td>
                                            <td>
                                                <div className="flex flex-col">
                                                    <span className="text-xs sm:text-sm font-medium text-charcoal">{asset.size}</span>
                                                    <span className="text-[9px] sm:text-[10px] text-brand-muted sm:hidden">{asset.dimension}</span>
                                                </div>
                                            </td>
